        ...
        <div class=" w-full mx-auto bg-white">
            <header class="px-5 py-4 border-b border-gray-100 flex justify-between items-center">
                ...
                @if (Auth::user()->role->role == 'admin')
                    <a href="{{ route('admin.salle.create') }}"
                        class="text-sm bg-[#c9ba46]  px-4 py-2 text-black font-bold">Ajouter</a>
                @endif
            </header>
            @include('components.toast')
            <div class="p-2">
                <form action="{{ route('admin.salle.recherche') }}" method="POST"
                                ...
                                <tr class="hover:bg-[#f9f9ed]">
                                    ...
                                    <td class="p-3 whitespace-nowrap">
                                        <div class="text-left h-14 w-16">
                                            <img class="w-full h-full" src="{{ asset($salle->reservable->image) }}">
                                        </div>
                                    </td>
                                    ...
                                    <td class="p-3 whitespace-nowrap">
                                        <div class="text-left">{{ $salle->reservable->etage }}</div>
                                    </td>
                                    <td class="p-3 whitespace-nowrap">
                                        <div class="text-left">{{ $salle->reservable->capacite }}</div>
                                    </td>
                                    <td class="p-3 whitespace-nowrap">
                                        <div class="text-left">
                                            {{ number_format($salle->reservable->tarif, '0', '.', ' ') }}</div>
                                    </td>
                                    <td class="p-3 whitespace-nowrap">
                                    ...
                                    <td class="p-3 whitespace-nowrap">
                                        <div class="text-end flex gap-2 justify-end">
                                            <a href="{{ route('admin.salle.edit', $salle) }}" class="btn btn-primary">
                                            ...
                                            {{-- @if (Auth::user()->role->role == 'admin')
                                                <form action="{{ route('admin.salle.destroy', $salle) }}"
                                                    method="post">
                                                    @csrf
                                                    @method('delete')
                                                    <button class="btn ">
