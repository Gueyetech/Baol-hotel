<x-admin>
    ...
    @section('title', 'Modifier le salle')
    ...
    <div class=" h-full">
        <div class="mx-auto max-w-3xl shadow-xl px-6 py-6 bg-white border-0  sm:rounded-sm">
            ...
            <form action="{{ route('admin.salle.update', $salle) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('get')
                ...
                @if (Auth::user()->role->role == 'admin')
                   <div class="relative z-0 w-full mb-5 text-center justify-center">
                        <div class=" mx-auto  max-w-64 gap-4 items-center w-fit ">
                            <img src="{{asset($salle->reservable->image)}}" alt="Image de profil" class=" h-52  profile-image">
                ...
                @endif
                <div class="grid grid-cols-1 gap-3 @if (Auth::user()->role->role == 'admin') sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 @endif">

                    @if (Auth::user()->role->role == 'admin')

                        <div class="relative z-0 w-full mb-5">
                            <input type="text" name="numero" placeholder=""
                                value="{{ old('numero', $salle->reservable->numero) }}"
                        ...
                        <div class="relative z-0 w-full mb-5">
                            <input type="number" name="etage" placeholder=" "
                                value="{{ old('etage', $salle->reservable->etage) }}"
                                class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200" />
                            <label for="etage"
                        ...
                        <div class="relative z-0 w-full mb-5">
                            <input type="text" name="capacite" placeholder=""
                                value="{{ old('capacite', $salle->reservable->capacite) }}"
                                class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200" />
                        ...
                        <div class="relative z-0 w-full mb-5">
                            <input type="text" name="tarif" placeholder=""
                                value="{{ old('tarif', $salle->reservable->tarif) }}"
                                class="pt-3 pb-2 block w-full px-0 mt-0 bg-transparent border-0 border-b-2 appearance-none focus:outline-none focus:ring-0 focus:border-black border-gray-200" />
                            <label for="numero" class="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">
                                Tarif</label>
                            @error('tarif')
                            ...
                            <select name="categorie"
                                ...
                                @foreach ($categories as $categorie)
                                    <option value="{{ $categorie->id }}"
                                        @if ($salle->reservable->categorie_id == $categorie->id) selected @endif>
                                        {{ $categorie->nom }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="relative z-0 w-full mb-5">
                        <select name="etat"
                            ...
                            @foreach ($etats as $etat)
                                <option value="{{ $etat->id }}" @if ($salle->reservable->etat_id == $etat->id) selected @endif>
                                    {{ $etat->etat }}
                                </option>
                            @endforeach
                        </select>
                        <label for="etat" class="absolute duration-300 top-3 -z-1 origin-0 text-gray-500">Etat du salle</label>
                        @error('etat')
                            <span class="text-sm text-red-600">
                                {{ $message }}
                            </span>
                ...
                @if (Auth::user()->role->role == 'admin')
                    <div class="w-full mb-5">
                        <label for="description" class=" duration-300  text-gray-500">
                            Description
                        </label>
                        <textarea type="text" name="description" placeholder=" "
                         ...
                         {{ old('description', $salle->reservable->description) }}
                        </textarea>
                        @error('description')
                            <span class="text-sm text-red-600">
                                {{ $message }}
